...
The lesson schema reflects an established ML paradigm: structured state capture paired with outcome data enables systematic learning from decisions. The approach has been explored across multiple domains – most recently in financial reasoning models trained on hundreds of thousands of decision traces.

What Engram brings to this paradigm:

- **Real traders, real capital.** Most academic work in this space relies on synthetic agents or backtested historical data. Engram captures decisions made by humans with their own money on the line – categorically different signal quality.
- **Multi-source context.** Research consistently shows that multi-source feature fusion outperforms single-source approaches in quantitative decision-making. The 15 categories span market structure, positioning, microstructure, sentiment, and trader psychology – each contributing independent signal.
- **Reasoning paired with outcome.** Capturing why a trader made a decision alongside what happened is the foundation of modern financial reasoning research. It's also how humans learn from their own trades.
